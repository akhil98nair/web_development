...
if(isset($_POST['forgotSubmit'])){
    ...
    if(!empty($_POST['email'])){
        ...
        $prevUser = $user_name->getRows($prevCon);
        if($prevUser > 0){
            ...
            $update = $user_name->update($data, $conditions);
            
            if($update){
                $resetPassLink = 'http://localhost/Turning_page/Turning_page/resetpassword.php?fp_code='.$uniqidStr;
                
                //get user details
                $con['where'] = array('email'=>$_POST['email']);
                $con['return_type'] = 'single';
                $userDetails = $user_name->getRows($con);
                
                //send reset password email
                $to = $userDetails['email'];
                $subject = "Password Update Request";
                $mailContent = 'Dear '.$userDetails['user_name'].', 
                ...
                //additional headers
                $headers .= 'From: princy<princy132nadar@gmail.com>' . "\r\n";
                //send email
